...
contract Pool is IPoolUsd3, Initializable, PausableUpgradeable {
    ...
    function borrow(uint128 loan, uint128 collateral, uint256 posId) external override onlyAdapter whenNotPaused {
        ...
        _validatePosition(loan, collateral, price);
    ...
    function borrowMore(
        ...
        (, commissionQuote) = _calculateLoanFees(position.loan, position.k, currentK, price);

        position.collateral = position.collateral + collateral - uint128(commissionQuote);
        position.loan += loan;
        position.k = currentK;

        _validatePosition(position.loan, position.collateral, price);
    }
        ...
        returns (uint256 liqLoan, uint128 liqColl, uint256 rewPlatform, uint256 rewLiquidator, uint256 commissionQuote)
    {
        ...
        (commissionBase, commissionQuote) = _calculateLoanFees(position.loan, position.k, currentK, price);
        uint256 collateralToBaseCurrency = _convertToBaseCurrency(position.collateral, price);
        ...
        (liqLoan, rewPlatform, rewLiquidator) = _calcLiquidation(
            position.loan,
            collateralToBaseCurrency - commissionBase
        );

        position.loan -= uint128(liqLoan);
        liqColl = uint128(_convertToQuoteCurrency(liqLoan, price));
        ...
        return (
            liqLoan,
            liqColl,
            _convertToQuoteCurrency(rewPlatform, price),
            _convertToQuoteCurrency(rewLiquidator, price),
            commissionQuote
        );
    }
    ...
    function extraLiquidate(
        ...
        lossQuote = lossBase > 0
            ? int256(_convertToQuoteCurrency(uint256(lossBase), price))
            : -int256(_convertToQuoteCurrency(uint256(-lossBase), price));
        delete _positionById[posId];
        ...
        returns (Position memory position, int256 healthFactor, uint256 commissionBase, uint256 commissionQuote)
    {
        ...
        uint256 currentK = _calcK();
        (commissionBase, commissionQuote) = _calculateLoanFees(position.loan, position.k, currentK, price);
        uint256 collateralToBaseCurrency = _convertToBaseCurrency(position.collateral - commissionQuote, price);
        healthFactor = _calcHealth(position.loan, collateralToBaseCurrency);
    ...
    function getPositionHealth(uint256 posId) public view override returns (int256 healthFactor) {
        ...
        return _calcHealth(position.loan, collateralToBaseCurrency - commissionBase);
    }
    ...
    function _validatePosition(uint256 loan, uint256 collateral, uint256 price) internal view {
        uint256 collateralToBaseCurrency = _convertToBaseCurrency(collateral, price);
    ...
    function _calculateLoanFees(
        ...
        commissionQuote = _convertToQuoteCurrency(commissionBase, price);
    }
    ...
    function _convertToBaseCurrency(
        uint256 amount,
        uint256 price
    ) internal view returns (uint256 amountInBaseCurrency) {
        uint256 convertPrice = (10 ** _decimalsToken1 * price) / 10 ** _dataFeedDecimals;
        amountInBaseCurrency = (amount * convertPrice) / 10 ** _decimalsToken1;
    }
    ...
    function _convertToQuoteCurrency(
        uint256 amount,
        uint256 price
    ) internal view returns (uint256 amountInQuoteCurrency) {
        return (amount * 10 ** _dataFeedDecimals) / price;
    }
